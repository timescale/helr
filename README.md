...
**Output:** Each NDJSON line (stdout) has a top-level **`source`** field (the config source key, e.g. `okta-audit`). Override with **`source_label`** per source for downstream labeling (e.g. Loki) or a stable name. When **log format is JSON** (`log_format: json` or `HEL_LOG_FORMAT=json`), Hel's own log lines (stderr) also include **`"source":"hel"`** so you can label/filter collector logs consistently with API events. Events are emitted in page order (order received). Timestamp order across pages is not guaranteed unless the API guarantees it; downstream can sort if needed.
