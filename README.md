...
See `hel.yaml` for a minimal example. Config path is per subcommand (e.g. `hel run --config hel.yaml`; default `hel.yaml`). Required: `sources` with at least one source (`url`, optional `schedule`, `auth`, `pagination`, `resilience`). Placeholders like `${OKTA_DOMAIN}` are expanded at load time. **Okta** System Log: API token (SSWS), link-header pagination — see **`docs/okta-manual.md`**. **HTTP method:** use **`method: post`** and **`body`** (JSON) for POST-only APIs (e.g. [Cloud Logging `entries.list`](https://cloud.google.com/logging/docs/reference/v2/rest/v2/entries/list)); with cursor pagination the cursor is merged into the body. Response arrays are read from top-level or from keys: **`items`**, **`data`**, **`events`**, **`logs`**, **`entries`** (Cloud Logging). **Google Workspace (GWS)** audit logs: use the Admin SDK Reports API (GET, cursor `pageToken`/`nextPageToken`) with OAuth2 or **service account + domain-wide delegation** — see commented `gws-login` in `hel.yaml` and **`docs/gws-gcp-manual.md`** (GWS & GCP manual). Corner-case options: **`from`** / **`from_param`** (start of range for first request), **`query_params`** (first-request params: limit, until, filter, q, sortOrder), `on_cursor_error`, `on_parse_error`, `on_invalid_utf8`, `on_state_write_error`, `max_response_bytes`, `max_line_bytes` / `max_line_bytes_behavior`, `checkpoint`, `dedupe.id_path`, `rate_limit.page_delay_secs`. Unset config placeholders and missing auth secrets fail at startup.

**Output:** Each NDJSON line (stdout) has a producer label field (key from **`global.source_label_key`**, default `"source"`; overridable per source with **`source_label_key`**). The value is the config source key (e.g. `okta-audit`) or **`source_label_value`** if set. When **log format is JSON** (`log_format: json` or `HEL_LOG_FORMAT=json`), Hel's own log lines (stderr) use the same key and **`global.source_label_value`** (default `"hel"`); tracing's **`target`** is renamed to **`module`** so the producer field is unambiguous. Events are emitted in page order (order received). Timestamp order across pages is not guaranteed unless the API guarantees it; downstream can sort if needed.
