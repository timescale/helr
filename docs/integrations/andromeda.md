...
- **Backfill:** Each poll requests one page at `state.skip` (starts at 0), then saves `skip += events.length`. Run `helr run` (or `helr test --source andromeda-audit` repeatedly) to walk through pages. When a page returns fewer than `pageSize` events, you've reached the end.
...
- **getNextPage(ctx, request, response):** Uses `request.body.variables.pageArgs.skip` from the request that was sent. If the response has a full page (`edges.length === PAGE_SIZE`), returns the next body with `skip: currentSkip + PAGE_SIZE` so Helr fetches the next page in the same poll. Returns `null` when no more pages.
